...
class Transforms:
    def __init__(self, size, s=1.0, mean=None, std=None, blur=False):
        ...
        if mean and std:
            ...
            self.train_week_transform.append(torchvision.transforms.Normalize(mean=mean, std=std))
        ...
        self.train_week_transform = torchvision.transforms.Compose(self.train_week_transform)

    def __call__(self, x):
        return self.train_transform(x), self.train_transform(x) , self.train_week_transform(x)
